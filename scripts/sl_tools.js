...
function getGridType(config, x, y){
	var _grid_type = config.mine_data[y][x].type;
	if (_grid_type == GRID_TYPE_EMPTY && config.mine_data[y][x].mines_around > 0)
	{
...
function game_init(config){
	...
	//生成格子数据
	_result.grid_data = genMinesData(_result);
	//设置大容器宽高
...
function selectLastClick(config, x, y){
	...
	{
		$('#' + config.last_clicked).css('border-color', '#000000');
	}
	config.last_clicked = '#' + getImgId(config, x, y);
}
function genMinesData(config){
var _data = [];
		addMsgTo('genMinesData...');
		var _time_start = new Date().getTime();
		var _data = [];
		var _line = [];
